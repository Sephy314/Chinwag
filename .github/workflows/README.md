...
| `cd.yml` | push to `main`, manual (`workflow_dispatch`) | Self-hosted runner on the k3s node syncs `/home/sephy/Chinwag` to `origin/main` and runs `infra/k3s/update.sh` |
...
1. **Persistent checkout** at `/home/sephy/Chinwag` (change `DEPLOY_DIR` in
   `cd.yml` if different). It must contain `infra/k3s/secret.yaml` (gitignored,
   so `git reset --hard` won't delete it):
   ```bash
   git clone git@github.com:Sephy314/Chinwag.git /home/sephy/Chinwag
