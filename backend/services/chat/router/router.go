...
func (r *Router) Setup(cfg *RouterConfig) {
	...
	priv.Use(sharedauth.NewMiddleware(jwksClient))
	{
		...
		priv.GET("/chat/rooms/:roomId/messages", r.ChatHandler.ListMessages)
		priv.GET("/chat/rooms/:roomId/messages/:messageId", r.ChatHandler.GetMessage)
	...
	r.log.Info("chat routes registered")
}
